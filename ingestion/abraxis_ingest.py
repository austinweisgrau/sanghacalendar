...
from ingestion.sources import bend as bend_sources  # noqa: F401 (no live feeds)
...
def main():
    ...
    # Tampa Bay Phase 3 — FCM Tampa Wild Apricot RSS
    log.info("--- Tampa Bay Phase 3: FCM Tampa Wild Apricot RSS ---")
    ...
    # Charlotte Phase 3 — all sits seeded as recurring; no iCal feeds
    log.info("--- Charlotte Phase 3: no iCal feeds (recurring sits only) ---")

    # Tucson Phase 3 — all sits seeded as recurring; no iCal feeds
    log.info("--- Tucson Phase 3: no iCal feeds (recurring sits only) ---")

    # Providence RI Phase 3 — Providence Zen Center Tockify ICS + recurring-only centers
    log.info("--- Providence RI Phase 3: iCal feeds ---")
    for org_id, feed_cfg in providence_sources.ICAL_FEEDS.items():
        center = providence_sources.CENTERS[org_id]
        log.info(f"Fetching {center.name} (Providence iCal)...")
        try:
            events = fetch_feed(
                url=feed_cfg["url"],
                org_id=org_id,
    ...
    # Tallahassee FL Phase 3 — Tallahassee Chan Center iCal feed
    log.info("--- Tallahassee FL Phase 3: Tallahassee Chan Center iCal feed ---")
    ...
    for org_id, feed_cfg in santa_barbara_sources.ICAL_FEEDS.items():
        center = santa_barbara_sources.CENTERS[org_id]
        log.info(f"Fetching {center.name} (Santa Barbara iCal)...")
        try:
            events = fetch_feed(
                url=feed_cfg["url"],
                org_id=org_id,
            ...
            log.error(f"  ✗ Santa Barbara iCal {org_id} failed: {e}")

    # Greenville SC Phase 3 — Greenville Zen Center iCal feed
    log.info("--- Greenville SC Phase 3: Greenville Zen Center iCal feed ---")
